...
checkLet e pos decs exps = do
  ...
  let e'' = unscope e'
  return $ T.Let pos (e'', retTyp) tdecs texps
...
checkListExps :: TypeEnv ->
                 [Expression] ->
                 Either TypeError [TExp]
checkListExps e exps = sequence $ typeCheck e <$> exps
...
declareEnv :: TypeEnv -> SourcePos -> [Declaration] -> Either TypeError (TypeEnv, [T.Declaration])
declareEnv e pos ds = declareEnv' pos ds (Right (e, []))

declareEnv' :: SourcePos ->
               [Declaration] ->
               Either TypeError (TypeEnv, [T.Declaration]) ->
               Either TypeError (TypeEnv, [T.Declaration])
declareEnv' _ [] res = (fmap . fmap) reverse res
declareEnv' _ _ err@(Left _) = err
declareEnv' pos (d:ds) (Right (e, tds)) = declareEnv' pos ds ((fmap . fmap) (addTail tds) $ declare e pos d)
  where addTail decs dec = dec:decs

declare :: TypeEnv ->
           SourcePos ->
           Declaration ->
           Either TypeError (TypeEnv, T.Declaration)
declare e pos (TypeDec name typ) = declareType e pos name typ
declare e pos (VarDec name typeName exp) = declareVariable e pos name typeName exp
declare e pos (FnDec name params typeName body) = declareFn e pos name params typeName body
...
declareVariable e pos name (Just typName) exp = do
    e' <- lookupType e typName pos
    texp <- verifyType' exp e'
    let e' = addVarBinding name (typeInfo texp)
    return (e', T.VarDec name texp)
